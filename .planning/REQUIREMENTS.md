...
- [x] **BUG-01**: `deleteLastEntry` aggiunge filtro `WHERE dateKey = :today` per evitare cancellazione cross-day (undo dell'ultima entry di ieri invece di oggi)
...
- [x] **BUG-03**: `dateKey` valida sia formato (regex YYYY-MM-DD) sia semantica (DateTime.tryParse), rifiutando date come 2024-02-30

### Target History

- [x] **TARGET-01**: Nuova tabella Drift `target_history (id, effectiveDate TEXT UNIQUE, targetMl INTEGER)` aggiunta allo schema iniziale (prima vera installazione — nessuna migration necessaria); seeded con il target default al primo avvio
