# Requirements Archive: v1.2 Bug Fixes & Feature Depth
...
- [x] **BUG-02**: La data corrente si aggiorna a mezzanotte senza richiedere restart dell'app (il provider con `_todayDateKey()` viene invalidato alla mezzanotte)
...
- [x] **TARGET-02**: Nuovo setting "Applica target da: oggi / da domani" — la scelta viene applicata ad ogni modifica del target nei Settings
- [x] **TARGET-03**: Home screen usa il target effettivo della giornata corrente (query su `target_history`) per il progress ring e il testo goal
- [x] **TARGET-04**: Calendario usa il target effettivo della giornata appropriata (query su `target_history`) per determinare verde/rosso per ogni giorno
...
| TARGET-02 | Phase 10 | Complete |
| TARGET-03 | Phase 10 | Complete |
| TARGET-04 | Phase 10 | Complete |
